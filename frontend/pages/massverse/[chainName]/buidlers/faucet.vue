...
const queryBSTBalance = async() => {
  const contractReader = await initContract('BuidlerStableToken')
...
const doSubmit = async() => {
  ...
  const contractWriter = await initContract('BuidlerStableToken', true)
      ...
      <div class="relative">
        <div class="sm:text-center">
          <h2 class="font-bold text-white tracking-tight text-3xl sm:text-4xl">
            Buidler Stable Token($BST) Faucet
          </h2>
          <p class="mx-auto mt-6 text-lg max-w-2xl text-indigo-200">
            The whole Buidlers Platform require Buidler ERC20 Token to pay as fee.
          </p>
        </div>
        ...
        <div v-if="faucetWalletAddress" class="mt-12 sm:text-center">
          <h2 class="font-bold text-white tracking-tight text-3xl sm:text-4xl">
            {{ shortAddress(faucetWalletAddress) }} have {{ humanFormatEther(balance) }} $BST
          </h2>
