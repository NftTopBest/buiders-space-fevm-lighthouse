...
const marketType = $computed(() => route.query.type)
...
<template>
  <div class="bg-white">
    <div class="flex mx-auto flex-1 max-w-7xl items-stretch overflow-hidden ">
          ...
          <!-- <div class="mt-3 sm:mt-2">
            ...
            <div class="hidden sm:block">
              <div class="border-b flex border-gray-200 items-center">
                ...
                <div class="rounded-lg bg-gray-100 ml-6 p-0.5 hidden items-center sm:flex">
                  <button type="button" class="rounded-md p-1.5 text-gray-400 hover:bg-white hover:shadow-sm focus:outline-none focus:ring-inset focus:ring-2 focus:ring-indigo-500">
                    <Bars4Icon class="h-5 w-5" aria-hidden="true" />
                    <span class="sr-only">Use list view</span>
                  </button>
          ...
          <section class="mt-8 pb-16" aria-labelledby="gallery-heading">
            ...
            <ul role="list" class="grid gap-x-4 gap-y-8 grid-cols-2 sm:gap-x-6 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-3 xl:gap-x-8 xl:grid-cols-4">
              <li v-for="item in items" :key="item.itemId" class="relative" :class="item.isListed ? '' : 'hidden'">
                ...
                <p class="font-medium text-sm text-gray-500 pointer-events-none block">
      ...
      <!-- <aside  class="bg-white border-l border-gray-200 p-8 w-96 hidden overflow-y-auto lg:block">
        <div class="space-y-6 pb-16">
          <div>
            <div class="rounded-lg w-full block overflow-hidden aspect-w-10 aspect-h-7">
              <img :src="currentitem.source" alt="" class="object-cover">
            </div>
            <div class="flex mt-4 items-start justify-between">
              ...
              <button type="button" class="bg-white rounded-full flex h-8 ml-4 text-gray-400 w-8 items-center justify-center hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500">
          ...
          <div>
            ...
            <dl class="divide-y border-t border-b divide-gray-200 border-gray-200 mt-2">
              <div v-for="key in Object.keys(currentitem.information)" :key="key" class="flex font-medium text-sm py-3 justify-between">
                <dt class="text-gray-500">
                  {{ key }}
                </dt>
                <dd class="text-gray-900 whitespace-nowrap">
                  {{ currentitem.information[key] }}
                </dd>
              </div>
            </dl>
          </div>
          ...
          <div class="flex">
            <button type="button" class="border border-transparent rounded-md font-medium bg-indigo-600 flex-1 shadow-sm text-sm text-white py-2 px-4 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
              Download
            </button>
            <button type="button" class="bg-white border rounded-md font-medium border-gray-300 flex-1 shadow-sm text-sm ml-3 py-2 px-4 text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
              Delete
            </button>
          </div>
        </div>
