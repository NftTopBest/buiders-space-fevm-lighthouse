...
<template>
  <div class="bg-white">
    <div>
                ...
                <form class="border-t border-gray-200 mt-4">
                  ...
                  <ul role="list" class="font-medium py-3 px-2 text-gray-900">
                    <li v-for="category in subCategories" :key="category.name">
                      <a :href="category.href" class="py-3 px-2 block">{{ category.name }}</a>
                    </li>
                  </ul>
                    ...
                    <DisclosurePanel class="pt-6">
                      <div class="space-y-6">
                        <div v-for="(option, optionIdx) in section.options" :key="option.value" class="flex items-center">
                          ...
                          <label :for="`filter-mobile-${section.id}-${optionIdx}`" class="flex-1 ml-3 min-w-0 text-gray-500">{{ option.label }}</label>
                        </div>
                      </div>
                    </DisclosurePanel>
                  </Disclosure>
      ...
      <main class="mx-auto max-w-7xl sm:px-6 lg:px-0">
        <div class="border-b flex border-gray-200 pt-6 pb-6 items-baseline justify-between">
