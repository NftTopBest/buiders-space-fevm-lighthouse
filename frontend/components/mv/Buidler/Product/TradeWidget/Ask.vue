...
const { storeJson, initContract, addSuccess, userData, getContractAddress } = $(web3AuthStore())
...
const doSubmit = async() => {
  ...
  const tx1 = await contract.setApprovalForAll(getContractAddress('BuidlerProtocol'), true)
  const rc1 = await tx1.wait()

  const tx = await contract.ask(tokenId, amount, parseEther(unitPrice), cid)
