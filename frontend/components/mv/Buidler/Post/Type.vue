<template>
  <div class="rounded-lg shadow-lg ring-black ring-1 ring-opacity-5 overflow-hidden">
    <div class="bg-white grid py-6 px-5 gap-6 relative sm:p-8 sm:gap-8 lg:grid-cols-2">
      <button
        v-for="item in typeList" :key="item.name" :to="item.href" class="rounded-lg flex -m-3 p-3 items-start hover:bg-gray-100" :class="[form.type === item.value ? 'bg-gray-100': '']" @click="form.type = item.value"
      >
        ...
        <div class="ml-4">
          <p class="font-medium text-base text-left text-gray-900">
            {{ item.name }}
          </p>
